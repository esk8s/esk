...
def test_secret(test_secret):
  ...
  old_secret = get_secret_from_spec(
  ...
  new_secret = get_secret_from_spec(
    new_spec.get('name'),
  ...
  if created_path.get_path() != old_secret.get_path():
    raise Exception(f"Path does not match. Created with { created_path.get_path() } but expected {old_secret.get_path()}")
...
def get_secret_from_spec(name, namespace, spec):
  config = {}
  ...
  return s_controller.get_object_for_backend(
    name,
    namespace,
    spec.get('backend'),
    spec.get('path'),
