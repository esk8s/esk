...
def should_mutate(annotations):
  ...
  return (
    annotations.get(__SKIP_ANNOTATION) is None or
    annotations.get(__SKIP_ANNOTATION).lower() != 'true'
  )

def mutate(controller, init_image, processor_addr, req):
  ...
  for bind in raw_binds:
    ...
    use_default_volume = use_default_volume or bind.get('target').startswith('/esk/secrets')
