...
from engine.main import ESKReaderEngine
...
engine = ESKReaderEngine()
processor = ESKProcessor(engine)
...
def mutate_webhook():
    allowed, patch = mutate(
        engine, init_image, processor_addr, request.json["request"]
    )
...
def read_secret():
    ...
    ret_code, values = processor.process_secret_read(
        data.get("auth"), data.get("name"), data.get("namespace") or __DEFAULT_NAMESPACE
    )
