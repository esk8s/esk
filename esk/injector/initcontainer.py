...
class InitContainer:
    def __init__(self, name, image, processor_addr):
        self.__name = name
        self.__image = image
        self.__env = [{"name": "PROCESSOR_ADDR", "value": processor_addr}]
        self.__vol_mounts = []
        self.__volumes = []
    ...
    def append_volumes_for_bind(self, bind):
        ...
        self.__vol_mounts.append(
            {
                "name": f"esk-secrets-{ bind.get('name') }",
                "mountPath": "/".join(bind.get("target").split("/")[0:-1]),
            }
        )

        self.__volumes.append(
            {"name": f"esk-secrets-{ bind.get('name') }", "emptyDir": {}}
        )
                ...
                "value": json.dumps(
                    {
                        ...
                        "namespace": namespace,
                        "secrets": ",".join(
                            [s.get("name") for s in spec.get("secrets")]
                        ),
                        "target": bind.get("target") or spec.get("target"),
                        "template": bind.get("template") or spec.get("template"),
                    }
                ),
            }
        )
